...
function fillUserGrid()
{
	...
	for (var i = 0; i < mulLineShowCount; i++)
	{
		...
		{//alert(userArray);
			UserGrid.setRowColData(i,1,userArray[offset][1]);//alert(userArray[offset][8]);
			...
			UserGrid.setRowColData(i,7,userArray[offset][8]);
			UserGrid.setRowColData(i,8,userArray[offset][9]);
...
function queryDetailProcess(Result)
{
	...
	for (var i = 0; i < tempArray.length; i++)
	{
		...
		userArray[i][19] = tempArray[i][20];
		userArray[i][20] = tempArray[i][21];		
	}
	fillUserGrid();
}
...
function beforeSubmit()
{
	...
	if (fm.all("UserCode").value.length > 10)
	{
		alert("用户编码不能大于10位");
		return false;
	}
...
function queryClick()
{
	if(fm.all("UserCode").value == "001")
	{
		alert("不能对001超级用户进行权限修改！");
		return false;
	}
