...
function MulLineEnter( iFormName , iInstanceName )
{
  ...
  this.newColOrder=new Array();//列的顺序 add by wanglei
	...
	this.DbclickToResize=_DbclickToResize;
...

{
...

	{
...

		{
          ...
          if( eval(tObj+"=='cssButton_s'"))
          {
          	eval(tObj+"='cssButton'");
          }
        	else	if( eval(tObj+"=='code8'")||eval(tObj+"=='codeselect'"))
...

			{
				...
				if( eval(tObj+"=='cssButton'"))
        {
          	eval(tObj+"='cssButton_s'");
        }
        	else	if( eval(tObj+"=='code8'")||eval(tObj+"=='codeselect'"))
...
//					text = text + "<td class=muline width='"+boxWidth+"'>";
      ...
      rate=(cObjInstance.windowWidth/userWidth)/cObjInstance.mulLineNum;
        ...
        if(tempText5==undefined||tempText5=="")tempText5=null;
        if(tempText6==undefined||tempText6=="")tempText6=null;
        if(tempText7==undefined||tempText7=="")tempText7=null;
        if(tempText8==undefined||tempText8=="")tempText8=null;
        if(tempText9==undefined||tempText9=="")tempText9=null;
        if(tempText10==undefined||tempText10=="")tempText10=null;
        if(tempText11==undefined||tempText11=="")tempText11=null;
        if(tempText12==undefined||tempText12=="")tempText12=null;
        if(tempText13==undefined||tempText13=="")tempText13=null;
        if(tempText14==undefined||tempText14=="")tempText14=null;
        if(tempText15==undefined||tempText15=="")tempText15=null;
        if(tempText16==undefined||tempText16=="")tempText16=null;
        if(tempText17==undefined||tempText17=="")tempText17=null;
        if(tempText18==undefined||tempText18=="")tempText18=null;
        if(tempText19==undefined||tempText19=="")tempText19=null;
        if(tempText20==undefined||tempText20=="")tempText20=null;
        if(tempText21==undefined||tempText21=="")tempText21=null;
        if(tempText22==undefined||tempText22=="")tempText22=null;
        if(tempText23==undefined||tempText23=="")tempText23=null;
        ...
        if(tempText1==0){
        	tempTextTitle1=0;
				...
				{
					textTitle = textTitle + "<td  class=mulinetitle> <input onmousedown=" + cObjInstance.instanceName + ".moveSpan(this,'"+strPageName+"',"+strPageName+") class=mulinetitle readonly value="
							+ tempText0 + " style='cursor:hand;width :" + tempTextTitle1 + "'>"
							+"<input onmousedown=" + cObjInstance.instanceName + ".mouseDownToResize(this); onmousemove=" + cObjInstance.instanceName + ".mouseMoveToResize(this,'"+strPageName+"'); "
							+" ondblclick=" + cObjInstance.instanceName + ".DbclickToResize(this,'"+strPageName+"'); "
							+" onmouseup=" + cObjInstance.instanceName + ".mouseUpToResize(this,"+strPageName+"); style='cursor:e-resize;width:1'>";
					
					text = text + "<td class=mulinetitle ><input type=button name=" + strPageName + i + " value='"+tempText5+"' class=cssButton ";
					var params = tempText7.split('|');
					var tFunStr = "onclick="+tempText6+"(";
					for(var p=0;p<params.length;p++)
					{
						tFunStr+="'para_"+strPageName+params[p]+"'";
			...
			else if(tempText4=='5')
			{
				textTitle = textTitle + "<td  class=mulinetitle> <input  class=mulinetitle readonly value="
							+ tempText0 + " style='cursor:hand;width :" + (tempTextTitle1+16) + "'>"
							+"<input style='cursor:e-resize;width:1'>";
					
					text = text + "<td class=muline ><input id=" + strPageName + i + "_$row_index$ name="+strPageName + i+
					//"class="+isReadOnlyClass(tempText4);
					" value='' class=coolDatePickerMulline ";
...
//			strText = replace(strText,"$SpanId$",spanID);
			...
			strText = strText.replace(/\$row_index\$/g,spanID);
...
//		fm.all('tt').value=strOldText;
	...
	try{afterAddOne(t_StrPageName);}catch(e){}
}
...
//			strText = replace(strText,"$SpanId$",spanID);
			...
			strText = strText.replace(/\$row_index\$/g,i);
...
//				}
				...
				k=j+1;
//alert(t_StrPageName + k + " value=''"+"||||||"+" value='"+cData[i-1][j]+"'");
				strText = strText.replace(t_StrPageName + k + " value=''",t_StrPageName + k + " value='"+cData[i-1][j]+"'");
				//strText = strText.replace("para_"+t_StrPageName+k,cData[i-1][j]);
				strText = replace(strText,"para_"+t_StrPageName+k,cData[i-1][j]);
				
				strText = replace(strText,"{data"+k+"}",cData[i-1][j]);
...
/*******************************
 *当鼠标变为双向箭头(可拖动列宽)时，双击自动适应宽度
 *add by luzhe 2008-09-10 教师节
*******************************/
function _DbclickToResize(obj,StrPageName)
{
	//alert(tCol);
	 tCol=getCellIndex(obj.parentElement);
  //这里要得到最大的长度，初步按照字符串判断
 var mainLength = 0;
 for(var i=0;i<eval(StrPageName).mulLineCount;i++)
 {
 	var mText = eval(StrPageName).getRowColData(i,tCol-1);
 	var tLength = getStrLength(mText);
 	if(tLength>mainLength)
 	{
 		mainLength = tLength;
 	}
 }
 var mTitle = this.arraySave3[tCol-1][0];
 var mLength = getStrLength(mTitle);
 if(mLength>mainLength)
 {
 	mainLength = mLength;
 } 
 newWidth=mainLength*7.5;
 if(newWidth>0)
 {
 	obj.parentElement.childNodes[0].style.width=newWidth;
 	for (var k=0;k<tab_alldata.length;k++)
