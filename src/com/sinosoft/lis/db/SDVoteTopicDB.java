...
/*
 * <p>ClassName: SDVoteTopicDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: PhysicalDataModel_1
 */
public class SDVoteTopicDB extends SDVoteTopicSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public SDVoteTopicDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		SDVoteTopicSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "SDVoteTopicDB";
	...
	public boolean delete()
	{
		...
		try
		{
			...
			if(this.getTopicNo() == null || this.getTopicNo().equals("null")) {
				...
				pstmt.setString(1, this.getTopicNo());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getVoteNo() == null || this.getVoteNo().equals("null")) {
				...
				pstmt.setString(2, this.getVoteNo());
			}
			if(this.getTopicTitle() == null || this.getTopicTitle().equals("null")) {
				pstmt.setNull(3, 12);
			} else {
				pstmt.setString(3, this.getTopicTitle());
			}
			if(this.getVoteType() == null || this.getVoteType().equals("null")) {
				pstmt.setNull(4, 12);
			} else {
				pstmt.setString(4, this.getVoteType());
			}
			if(this.getDisplayFlag() == null || this.getDisplayFlag().equals("null")) {
				pstmt.setNull(5, 12);
			} else {
				pstmt.setString(5, this.getDisplayFlag());
			}
			...
			if(this.getTopicNo() == null || this.getTopicNo().equals("null")) {
				...
				pstmt.setString(21, this.getTopicNo());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM SDVoteTopic WHERE  TopicNo = ?", 
	...
	public SDVoteTopicSet query()
	{
		Statement stmt = null;
		ResultSet rs = null;
		SDVoteTopicSet aSDVoteTopicSet = new SDVoteTopicSet();
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			SQLString sqlObj = new SQLString("SDVoteTopic");
			SDVoteTopicSchema aSchema = this.getSchema();
			...
			{
				i++;
				SDVoteTopicSchema s1 = new SDVoteTopicSchema();
				s1.setSchema(rs,i);
				aSDVoteTopicSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
		...
		return aSDVoteTopicSet;
	}

	public SDVoteTopicSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		SDVoteTopicSet aSDVoteTopicSet = new SDVoteTopicSet();
	...
	public SDVoteTopicSet query(int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		SDVoteTopicSet aSDVoteTopicSet = new SDVoteTopicSet();
	...
	public SDVoteTopicSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		SDVoteTopicSet aSDVoteTopicSet = new SDVoteTopicSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update SDVoteTopic " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return SDVoteTopicSet
 */
public SDVoteTopicSet getData()
{
    int tCount = 0;
    SDVoteTopicSet tSDVoteTopicSet = new SDVoteTopicSet();
    SDVoteTopicSchema tSDVoteTopicSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "SDVoteTopicDB";
    ...
    {
        tCount = 1;
        tSDVoteTopicSchema = new SDVoteTopicSchema();
        tSDVoteTopicSchema.setSchema(mResultSet, 1);
        tSDVoteTopicSet.add(tSDVoteTopicSchema);
