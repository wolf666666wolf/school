...
public class DBConn implements java.sql.Connection
{
    ...
    public boolean createConnection()
    {
        ...
        try
        {
            if (con != null)
            {
                if (!con.isClosed())
                {
                    try
                    {
                        ...
                        stmt.execute("SELECT * FROM ldcom");
                        stmt.close();
                        return true;
                    }
                    catch (SQLException e)
                    {
                        log.addlog("DBConn Exception1: "+e.getMessage());
                        e.printStackTrace();
                        log.addlog("DBConn : recreate DBConn");
                        ...
                        {
                            log.addlog("DBConn Exception2: "+ex.getMessage());
                            e.printStackTrace();
                        }
                        finally
                        {
            ...
            else if (JUrl.getDBType().equalsIgnoreCase("SQLITE"))
            {
                dbType = 6;
            }
            ...
            switch (dbType)
            {
                ...
                case 6:
                    ...
                    break;
                default:
                    log.addlog("目前暂不支持此种类型的数据库!");
                    return false;
            }
        }
        catch (Exception e)
        {
            log.addlog("DBConn Exception3: "+e.getMessage());
            return false;
        }
        // 尝试连接数据库
        try
...
//                    log.addlog("###########DBConn getJdbcUrl(): "+JUrl.getJdbcUrl());
...
//                            , JUrl.getPassWord());
                ...
                case 5:
                    ...
                    con = DriverManager.getConnection(JUrl.getJdbcUrl(), JUrl.getUserName()
                            , JUrl.getPassWord());
                case 6:
//                    System.out.println("JdbcUrl"+JUrl.getJdbcUrl());
                        // SQLITE
                    	Connection conn = DriverManager.getConnection(JUrl.getJdbcUrl());
...
//                        con = DriverManager.getConnection(JUrl.getJdbcUrl());
        ...
        {
            log.addlog("创建连接失败..."+e.getMessage());
            return false;
        }
        return true;
    }
    ...
    private boolean getWeblogicPoolConnection()
    {
        ...
        {
            log.addlog("$$$$$$$$WebLogicPool Connect Failed$$$$$");
            return false;
        }
...
//        	Context mContext = (Context) tContext.lookup("java:comp/env");
////            Object obj = tContext.lookup(JUrl.getDBName());
////            DataSource tDataSource = (DataSource) obj;
//            DataSource tDataSource = (DataSource) mContext.lookup(JUrl.getDBName());
            
            InitialContext initCtx = new InitialContext();
            Context envCtx = (Context) initCtx.lookup("java:comp/env");
            DataSource tDataSource = (DataSource) envCtx.lookup(JUrl.getDBName());
...
//                        stmt.close();
            ...
            {
                log.addlog("a error occured when geting datasource");
                return false;
            }
        }
        catch (Throwable e)
        {
            log.addlog("failure when connect apache commons dbcp ");
            e.printStackTrace();
            return false;
        }
    }
...
//            res-resolution-control:   999 (undefined)
            if (tDataSource != null)
            {
                ...
                if (con != null)
                {
                    log.addlog("Connect succeed from websphere!");
                    return true;
                }
                else
                {
                    log.addlog("new Connection error ...");
                    return false;
                }
            }
            else
            {
                log.addlog("new DataSource error ...");
                return false;
            }
        }
        catch (Throwable e)
        {
            log.addlog("look for jndi name error ..."+JUrl.getDBName());
            e.printStackTrace();
            return false;
        }
    }
    ...
    public String getClientInfo(String s){return null;}
    ...
    public  void createArrayOf(String s){}
    public  Array createArrayOf(String s,Object[] o){return null;}
    public  Struct createStruct(String s,Object[] o){return null;}
